        ...
        : (
          <>
            <div className='flex mv4 justify-around'>
              <StatBox label='Confirmed cases' stat={countryData.latest_data.confirmed.toLocaleString()} />
              <StatBox label='Recovered cases' stat={countryData.latest_data.recovered.toLocaleString()} />
              <StatBox
                label='Recovery rate'
                stat={formatPercentage(countryData.latest_data.calculated.recovery_rate)}
              />
              <StatBox
                label='Death rate'
                stat={formatPercentage(countryData.latest_data.calculated.death_rate)}
              />
            </div>
              ...
              <tbody>
                {countryData.timeline.map(dataByDay => (
                  <tr className='stripe-dark f6' key={dataByDay.updated_at}>
                    <td className='pv3 pr3'>{format(dataByDay.date, 'LLL d, yyyy')}</td>
                    <td className='pv3 pr3'>{dataByDay.new_confirmed.toLocaleString()}</td>
                    <td className='pv3 pr3'>{dataByDay.active.toLocaleString()}</td>
                    <td className='pv3 pr3'>{dataByDay.recovered.toLocaleString()}</td>
                    <td className='pv3 pr3'>{dataByDay.new_deaths.toLocaleString()}</td>
                    <td className='pv3 pr3'>{dataByDay.deaths.toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
